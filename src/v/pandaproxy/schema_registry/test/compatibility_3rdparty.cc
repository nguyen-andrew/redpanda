...
SEASTAR_THREAD_TEST_CASE(test_consume_to_store_3rdparty) {
    ...
    // Test mode default
    BOOST_REQUIRE_EQUAL(
      c._store.get_mode(pps::default_context, pps::default_to_global::yes).get(), pps::mode::read_write);

    // Test mode READONLY
    BOOST_REQUIRE_NO_THROW(
      c(make_record_batch(mode_key_0, mode_value_ro, base_offset++)).get());
    BOOST_REQUIRE_EQUAL(
      c._store.get_mode(pps::default_context, pps::default_to_global::yes).get(), pps::mode::read_only);
