...
convert_to_pb_role_member(const security::role_member& role_member) {
    ...
    if (member_type == security::role_member_type::user) {
...
security_service_impl::create_scram_credential(
  ...
  proto::admin::create_scram_credential_request req) {
    ...
    // here. Commenting this out for now. auto& cred_store =
    // _controller->get_credential_store().local(); auto user_opt =
    // cred_store.get<security::scram_credential>(name); if
    // (user_opt.has_value() && user_opt.value() == credential) {
    //     vlog(
    //       securitylog.debug,
...
security_service_impl::create_role(
  serde::pb::rpc::context ctx, proto::admin::create_role_request req) {
    ...
    const auto& req_role_name = req_role.get_name();
    validate_role_name(req_role_name);
    const security::role_name role_name{req_role_name};

    // Question: Should we also validate role member names?
    ...
    if (err == cluster::errc::role_exists) {
        ...
        vlog(
          ...
          err);
        throw serde::pb::rpc::unknown_exception(
          ssx::sformat("Failed to create role '{}'", req_role_name));
    }
...
security_service_impl::get_role(
  serde::pb::rpc::context, proto::admin::get_role_request req) {
    ...
    const auto& req_role_name = req.get_name();
    validate_role_name(req_role_name);
    const security::role_name role_name{req_role_name};
    ...
    res.set_role(convert_to_pb_role(req_role_name, role));
    co_return res;
}
...
security_service_impl::add_role_members(
  serde::pb::rpc::context ctx, proto::admin::add_role_members_request req) {
    ...
    const auto& req_role_name = req.get_role_name();
    validate_role_name(req_role_name);
    const security::role_name role_name{req_role_name};
