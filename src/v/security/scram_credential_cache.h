...
/// inputs so plaintext passwords are not retained, and hold only the derived
/// stored key: the same datum already persisted in the credential store, so
/// the cache adds no key material a memory disclosure would not already
/// yield. The memoized mapping is purely functional, so entries never go
/// stale: updating a credential generates a fresh salt, which changes the
/// cache key.
...
private:
    ...
    struct key_t {
        std::array<char, key_digest_size> digest;
