...
// Encode `original` at version 0, decode into a fresh value, verify equality.
// Takes `original` by mutable reference because the generated encode() is
// non-const; encoding does not modify it.
template<typename T>
static void round_trip(T& original) {
    iobuf buf;
    {
