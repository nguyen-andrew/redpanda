...
public:
    ...
    /// Create a role on the destination. errc::role_exists if it already
    /// exists; errc::feature_disabled if RBAC is not active.
    virtual ss::future<std::error_code> create_role(
      security::role_name,
      security::role,
      ::model::timeout_clock::duration) = 0;
    /// Overwrite a role's membership on the destination.
    ...
    virtual ss::future<std::error_code> update_role(
      ...
      security::role,
      ::model::timeout_clock::duration) = 0;
    /// Delete a role on the destination. errc::role_does_not_exist if already
    /// gone; errc::feature_disabled if RBAC is not active.
    ...
    /// Enumerate destination roles (with members) matching the predicate,
    /// from the local controller role_store.
    virtual chunked_vector<security::role_with_members> read_destination_roles(
