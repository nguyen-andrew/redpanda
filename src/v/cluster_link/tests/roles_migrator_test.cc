...
TEST_F_CORO(roles_migrator_test, fail_safe_on_fetch_error) {
    ...
    // The task must actually reach faulted (not merely time out)...
...
TEST_F_CORO(roles_migrator_test, no_filters_syncs_nothing) {
    ...
    // the destination is left untouched. (paused is reserved for user action,
    // so the task simply stays idle.)
    fixture().security_service().seed_role(
...
TEST_F_CORO(roles_migrator_test, link_unavailable_when_rbac_inactive) {
    // RBAC inactive on the destination is a preflight gate: the migrator parks
    // in link_unavailable and mirrors nothing, even with an in-scope source
    // role waiting to sync.
...
TEST_F_CORO(roles_migrator_test, filter_change_rescopes_sync) {
    ...
    // scope is orphaned -- it survives on the destination even once the source
    // drops it, whereas a still-in-scope role is pruned. Filtering is symmetric
    // across source and destination, so the deselected role is absent from both
    // reconcile snapshots and is never considered for deletion. (Were the
    // filter applied to the source only, the orphan would appear in the
    // destination snapshot but not the source one and get pruned.)
    set_source_roles(
      {rwm("synced-role", {user("u1")}),
       rwm(
         "rescoped-role",
