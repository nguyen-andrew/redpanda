...
roles_migrator::run_impl(ss::abort_source& as) {
    ...
    if (!get_link()->get_security_service().rbac_active()) {
        co_return state_transition{
          ...
          .reason = "RBAC disabled on destination (cannot sync roles)"};
    }
    ...
    auto destination_selected
      = get_link()->get_security_service().read_destination_roles(predicate);

    auto changes = reconcile_roles(
      std::move(source_selected), std::move(destination_selected));
    ...
    co_await ss::max_concurrent_for_each(
      ...
      [&](security::role_with_members& r) -> ss::future<> {
          if (rbac_disabled) {
              co_return;
          }
    ...
    co_await ss::max_concurrent_for_each(
      changes.to_delete,
      role_apply_concurrency,
      [&](security::role_name& name) -> ss::future<> {
          if (rbac_disabled) {
              co_return;
          }
    ...
    if (rbac_disabled) {
        co_return state_transition{
          .desired_state = model::task_state::link_unavailable,
          .reason = ssx::sformat(
            "RBAC disabled on destination ({} before RBAC was disabled)",
            summary)};
    }
