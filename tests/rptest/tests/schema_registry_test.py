...
class SchemaRegistryAclAuthzUnavailableTest(SchemaRegistryAclAuthzTestBase):
    ...
    def test_get_schemas_ids_id_unavailable_schemas_partition(self):
        ...
        # Stop the other two brokers. The target is then left below quorum, so
        # it can neither reach the old leader nor win an election, and
        # read_sync() has no leader to query.
        for node in self.redpanda.nodes:
            if node != target:
