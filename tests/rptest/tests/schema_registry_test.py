...
class SchemaRegistryContextTest(SchemaRegistryEndpoints):
    ...
    def test_reserved_subject_names_rejected(self):
        ...
        result = self.sr_client.get_subjects_subject_versions(subject="__GLOBAL")
        self.assert_equal(result.status_code, 422)
        self.assert_equal(result.json()["error_code"], 42208)
        ...
        result = self.sr_client.get_config_subject(subject=":.__GLOBAL:", fallback=True)
        self.assert_equal(result.status_code, requests.codes.ok)

        # .__GLOBAL context is allowed on mode endpoints
        result = self.sr_client.get_mode_subject(subject=":.__GLOBAL:", fallback=True)
        self.assert_equal(result.status_code, requests.codes.ok)
