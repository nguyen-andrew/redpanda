...
SCRAM_MECHANISM_MAP = {
    "SCRAM_MECHANISM_UNSPECIFIED": security_pb2.ScramCredential.SCRAM_MECHANISM_UNSPECIFIED,
    "SCRAM-SHA-256": security_pb2.ScramCredential.SCRAM_SHA_256,
    "SCRAM-SHA-512": security_pb2.ScramCredential.SCRAM_SHA_512,
}


def scram_mechanism_from_string(
    name: str,
) -> security_pb2.ScramCredential.ScramMechanism:
    try:
        return SCRAM_MECHANISM_MAP[name]
    except KeyError:
        return security_pb2.ScramCredential.SCRAM_MECHANISM_UNSPECIFIED
        raise ValueError(f"Unknown SCRAM mechanism: {name}")
...
class BaseScramTest(RedpandaTest):
    ...
    def update_scram_credential_v2(
        ...
        mechanism: security_pb2.ScramCredential.ScramMechanism = security_pb2.ScramCredential.SCRAM_SHA_256,
        ...
        if expected_error is not None:
            with expect_exception(
                ...
                _ = self.admin.security().update_scram_credential(req)
        else:
            _ = self.admin.security().update_scram_credential(req)
    ...
    def delete_scram_credential_v2(
        self, name: str, quote: bool = False, delete_acls: bool = False
    ) -> None:
        if quote:
            name = urllib.parse.quote(name, safe="")

        _ = self.admin.security().delete_scram_credential(
            security_pb2.DeleteRoleRequest(name=name, delete_acls=delete_acls)
        )
    ...
    def create_scram_credential_v2(
        self,
        name: str,
        mechanism: security_pb2.ScramCredential.ScramMechanism,
        password: str = None,
        expected_error: ConnectErrorCode | None = None,
    ) -> str:
        if password is None:
...
class ScramTest(BaseScramTest):
    ...
    def test_scram(self, use_v2_api):
        ...
        # recreate user
        if use_v2_api:
            password = self.create_scram_credential_v2(
                username,
        ...
        # update password
        if use_v2_api:
            new_password = self.update_scram_credential_v2(username)
        else:
...
class ScramBootstrapUserTest(RedpandaTest):
    ...
    def test_bootstrap_user_v2(self, mechanism):
        ...
        users = [
            ...
            .list_scram_credentials(security_pb2.ListCurrentUserRolesRequest())
            .scram_credentials
        ]
        ...
        admin.security().update_scram_credential(
            security_pb2.UpdateScramCredentialRequest(
                scram_credential=security_pb2.ScramCredential(
                    ...
                    mechanism=security_pb2.ScramCredential.ScramMechanism.SCRAM_SHA_256,
                    password=new_password,
                )
            )
        )
...
class EscapedNewUserStrings(BaseScramTest):
    ...
    def test_update_delete_user(self, use_v2_api):
        ...
        for username in users:
            if use_v2_api:
                self.update_scram_credential_v2(name=username, quote=False)
                self.delete_scram_credential_v2(name=username, quote=False)
            else:
